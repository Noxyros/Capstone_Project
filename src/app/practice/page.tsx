  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 pb-10">
      ...
      <div className="space-y-4">
        {practiceModes.map((mode) => {
          ...
          return (
            <div
              key={mode.id}
              className="bg-white border-2 border-b-4 border-slate-200 rounded-2xl p-4 sm:p-5 flex items-center gap-4 hover:bg-slate-50 active:border-b-2 active:translate-y-[2px] transition-all cursor-pointer select-none"
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-xs font-extrabold text-indigo-500 bg-indigo-50 px-2.5 py-1 rounded-lg uppercase tracking-wide">
                    {mode.badge}
                  </span>
                </div>
      ...
      <div className="pt-4">
        <Link
          href="/"
          className="inline-block font-extrabold text-slate-400 uppercase tracking-widest text-sm hover:text-indigo-500 transition-colors"
        >
          ← {t('Go to Learn', 'Kembali ke Belajar')}
        </Link>
      </div>
    </div>
  )
